...
async function fetchFirstAvailableJson(candidates) {
  ...
  return { filename: null, data: null, failures };
}
...
async function loadConfig() {
  ...
  // v4.9.18: resolve versioned calibration modules first,
  // while retaining compatibility with legacy unversioned names.
  const optionalGroups = [
    {
      key: "reliability_engine_v2",
      candidates: [
        "reliability_engine_v2_1.json",
        "reliability_engine_v2.json"
      ]
    },
    {
      key: "confidence_engine_v2",
      candidates: [
        "confidence_engine_v2_2.json",
  ...
  const optionalResults = await Promise.all(
    optionalGroups.map(group =>
      fetchFirstAvailableJson(group.candidates)
    )
  );

  const optionalParts = optionalResults
  ...
  const optionalConfigStatus = Object.fromEntries(
  ...
  const configWarnings = optionalGroups
    .map((group, index) => ({ group, result: optionalResults[index] }))
    .filter(({ result }) => !result.data)
    .map(({ group, result }) => ({
      type: "optional_calibration_config_missing",
      key: group.key,
      attempted_files: group.candidates,
      failures: result.failures
    }));
...
function stabilizeTemperatureResult(result, extractorData) {
  ...
  // v4.9.10: contradictory skin/eye temperature evidence must reduce
...
function baseScore(config, profile, dims, seasonId) {
  ...
  function scoreSeparation(result) {
    ...
    // v4.9.10: stability/reliability guards may intentionally cap
...
const {config,files,configWarnings,optionalConfigStatus}=await loadConfig();
...
return {
  ...
  loaded_files: files,
  optional_config_status: optionalConfigStatus,
